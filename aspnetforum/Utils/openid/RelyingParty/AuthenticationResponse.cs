namespace aspnetforum.Utils.openid.RelyingParty {
	...
	using aspnetforum.Utils.openid.Extensions;
	...
	internal class AuthenticationResponse : IAuthenticationResponse, ISetupRequiredAuthenticationResponse {
		...
		internal string GetExtensionClientScript(Type extensionType) {
			...
			if (!typeof(openid.Extensions.IClientScriptExtensionResponse).IsAssignableFrom(extensionType))
		...
		public IExtensionResponse GetExtension(Type extensionType) {
			if (extensionType == null) throw new ArgumentNullException("extensionType");
			if (!typeof(openid.Extensions.IExtensionResponse).IsAssignableFrom(extensionType))
				throw new ArgumentException(string.Format(CultureInfo.CurrentCulture,
